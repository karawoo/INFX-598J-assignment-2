---
title: "Washington State Domestic Violence Fatalities"
output:
  html_document:
    fig_width: 10
    fig_height: 7
...
```{r fatality_hist_by_year, fig.height = 9}
...
```{r fatality_hist_by_year_pop, fig.height = 9}
tot_fat_by_county_year %>%
...
```{r fatalities_line_by_type, fig.height = 6}
tot_fat_by_type %>%
...
```{r fatalities_stacked_area, fig.height = 6}
tot_fat_by_type %>%
...
```{r homicide_line_facet_by_county, fig.height = 10}
dat %>%
  filter(type == "homicide") %>%
...
```{r fatality_line_facet_by_county, fig.height = 10}
tot_fat_by_county_year %>%
...
```{r fatality_line_facet_by_county_pop_norm, fig.height = 10}
tot_fat_by_county_year %>%
  mutate(tot_fatal = tot_fatal / (pop / 10000)) %>%
  ggplot(aes(x = year, y = tot_fatal)) +
  facet_wrap(~ county, ncol = 5) +
...
```{r fatality_map_total, fig.height = 15}
...
```{r fatality_map_by_pop, fig.height = 15}
...
```{r scatterplot_by_year, fig.height = 13}
...
```{r scatterplot_by_year_labeled, fig.height = 13}
...
```{r map_single_sum, fig.cap = "Fig. 1. Total domestic violence homicides, suicides, and fatal police interventions between 1997 and 2014."}
tot_fat_by_county_year %>%
  group_by(county) %>%
...
```{r map_single_by_pop, fig.cap = "Fig. 2. Domestic violence fatalities per 10,000 population, averaged across 1997 to 2014."}
tot_fat_by_county_year %>%
  group_by(county) %>%
